			...
			<div class="container">
				...
				<div id="mainMenu" class="clearfix">
					<div class="btn-toolbar pull-left">
						<a href="month/meeting/index?type=3" class="btn btn-link ${prm.type == 3 ? 'btn-active-text':''}">
						...
						<a href="month/meeting/index?type=0" class="btn btn-link ${prm.type == 0 ? 'btn-active-text':''}">
							<span class="text">已删除</span>
							<c:if test="${prm.type == 0}">
								<span class="label label-light label-badge">${pageList.totalCounts}</span>
							</c:if>
						</a>
						<a href="month/meeting/index?type=2" class="btn btn-link ${prm.type == 2 ? 'btn-active-text':''}">
				...
				<div id="mainContent" class="main-row fade in">
					...
					<div class="main-col">
						<div class="cell load-indicator ${prm.type == 10 ? 'show':''}" id="queryBox">
							<form method="post" action="month/meeting/index?type=10" id="searchForm" class="search-form">
										...
										<tr>
											<td style="width:150px">
												<select class="form-control chosen chosen-select" name="searchType" id="searchType">
													<option ${prm.searchType=='1'?'selected="selected"':'' } value="1">会议名称/ID</option>
													<option ${prm.searchType=='2'?'selected="selected"':'' } value="2">会议备注</option>
												</select>
											</td>
								...
								<table class="table has-sort-head" id="taskList"
									...
									<thead>
										<tr>
											...
											<th data-flex="false" data-width="50px" style="width: auto" class="c-pri " title="会议名称">
											...
											<th data-flex="false" data-width="auto" style="width:250px" class="c-name text-center" title="需求总数">
												<a href="javascript:void(0)" onclick="pageOrder('totalCount');" 
														class="${prm.orderColumn=='totalCount'?(prm.orderByValue=='DESC'?'sort-down':'sort-up'):'header'}">需求总数</a>
											</th>
											<th data-flex="false" data-width="auto" style="width:200px" class="c-name text-center" title="已验收需求数">
												<a href="javascript:void(0)" onclick="pageOrder('approvedCount');" 
														class="${prm.orderColumn=='approvedCount'?(prm.orderByValue=='DESC'?'sort-down':'sort-up'):'header'}">已验收需求数</a>
											</th>
											<th data-flex="false" data-width="auto" style="width:200px" class="c-name text-center" title="待验收需求数">
												<a href="javascript:void(0)" onclick="pageOrder('waitApprovedCount');" 
									...
									<tbody>
										<c:forEach items="${pageList.pageResult}" var="meeting" varStatus="sta">
										<tr>
											...
											<td class="c-actions text-center">
												<c:if test="${meeting.state == '1'}">
													<a href="month/meeting/toAdd?id=${meeting.id}" class="btn" title="编辑"><i class="icon-common-edit icon-edit"></i> 编辑</a>
													<a href="javascript:void(0)" onclick="del(${meeting.id})" class="btn" title="删除"><i class="icon-common-delete icon-trash"></i> 删除</a>
												</c:if>
												<c:if test="${meeting.state != '1'}">--</c:if>
											</td>
										</tr>
										</c:forEach>
									</tbody>
