			...
			<div class="container">
				...
				<div id="mainMenu" class="clearfix">
					<div class="btn-toolbar pull-left">
						...
						<a href="my/need?type=16" class="btn btn-link ${prm.type == 16? 'btn-active-text':''}">
							<span class="text">由我安排</span>
							<c:if test="${prm.type == 16}">
								<span class="label label-light label-badge">${pageList.totalCounts}</span>
							</c:if>
						</a>
										...
										<tr>
											...
											<td class="c-actions text-right">
												...
												<c:if test="${need.full == 1}">
													...
													<c:if test="${need.resolved == 0}">
														<c:if test="${need.task_sum > 0 && need.not_finish_task == 0}">
														  <c:if test="${need.project_id=='0' && need.state == 2 && need.test_state == 4 && (need.parent_id == null || need.parent_id == 0)}">
														  ...
														  <c:if test="${need.project_id=='0' && need.state == 2 && need.parent_id > 0}">
														    <a href="my/need/toSubmitCheck?id=${need.id}" class="btn" data-toggle="tooltip" data-placement="top" title="提交验收"><i class="icon-task-finish icon-checked"></i></a>
														  </c:if>
												...
												<c:if test="${need.project_id=='0' && need.state==2 && need.prototype_figure==0 && need.resolved == 1 && (need.parent_id == null || need.parent_id == 0)}">
													<a href="my/need/toConfirmPrototypeFigure?id=${need.id}" class="btn" data-toggle="tooltip" data-placement="top" title="确认原型图"><i class="icon-task-finish icon-checked"></i></a>
												</c:if>
												<c:if test="${need.project_id=='0' && need.full == 1 && need.state == 2 && need.prototype_figure==1 && need.resolved == 0}">
												...
												<c:if test="${need.project_id!='0' && need.full == 1 && need.state == 2}">
												    <a href="my/task/toAdd?need_id=${need.id}" class="btn" data-toggle="tooltip" data-placement="top" title="建任务"><i class="icon icon-plus"></i></a>
												</c:if>
