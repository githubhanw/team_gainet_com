...
<html>
									...
									<td class="required">
										<select data-placeholder="请选择要关联是需求" class="form-control chosen chosen-select" multiple name="needs" id="needs">
											...
											<c:forEach items="${needs}" var="p" varStatus="sta">
												...
												<c:forEach items="${fn:split(n.link, ',')}" var="needId" varStatus="sta">
													<c:if test="${p.id == needId }">
														<option value="${p.id}" selected="selected">${p.need_name }</option>
														<c:set var="isSelected" value="1" />
													</c:if>
												</c:forEach>
												<c:if test="${isSelected == 0}">
													<option value="${p.id}">${p.need_name }</option>
												</c:if>
											</c:forEach>
										</select>
									</td>
	...
	$("#submit").click(function(){
		...
		$.ajax({type:"POST",url:"my/need/relevance?r=" + Math.random(),data:"id=" + $("#id").val() + "&needs=" + ($("#needs").val() == null ? "" : $("#needs").val()) + "&comment=" + UM.getEditor('comment').getContent(),dataType:"json",success:function(data){
			if(data.code == 0){
