						...
						<table class="table table-form">
							<tbody>
								...
								<tr>
									<th>父权限名称</th>
									<td class="required">
										<select class="form-control chosen-select" name="parent_id" id="parent_id" onchange="selectParent()">
											<option value="0">父级权限</option>
											<c:forEach items="${parentList}" var="parent" varStatus="sta">
												<option value="${parent.id}" ${parent.id==p.parentId?'selected="selected"':''}>${parent.name}</option>
											</c:forEach>
										</select>
									</td>
									<td></td>
								</tr>
								<tr id="privilegeUrl" ${p.id==null || p.parentId==0?'style="display:none"':''}>
									<th>权限URL</th>
									<td class="required">
										<input type="text" name="url" id="url" value="${p.url}" class="form-control input-product-title" autocomplete="off">
									</td>
									<td></td>
								</tr>
				...
				<div class="modal-content">
					...
					<div class="modal-body">
						<div style="margin:0 auto">
							...
							<div>
								<a href="organization/privilege/toAdd" class="btn">继续新建权限</a> <a
									href="organization/privilege/index" class="btn">返回权限列表</a>
							</div>
						</div>
					</div>
				</div>
...
$("#submit").click(function(){
	...
	$.ajax({type:"POST",url:"organization/privilege/addOrUpd?r=" + Math.random(),data:$("form").serialize(),
