...
<html>
	<head>
		...
		<title>产品详情：${p.productName}</title>
		...
		<main id="main">
			<div class="container">
				<div id="mainMenu" class="clearfix">
					<div class="btn-toolbar pull-left">
						...
						<div class="page-title">
							<span class="label label-id">${p.id}</span> <span class="text">${p.productName}</span>
						</div>
					</div>
				</div>
							...
							<tbody>
								...
								<tr>
									...
									<td>
										...
										<c:if test="${p.state=='2'}">产品待验收</c:if>
										<c:if test="${p.state=='3'}">产品已验收</c:if>
										<c:if test="${p.state=='4'}">产品已完成</c:if>
				...
				<c:if test="${needTask != null}">
					<div class="detail">
						<div class="detail-title">同产品模块</div>
				...
				<div id="mainActions">
					...
					<div class="btn-toolbar">
						<a href="team/product/index" class="btn btn-link"><i class="icon-goback icon-back"></i> 返回</a>
						<div class="divider"></div>
						<a href="team/product/toedit?id=${p.id}" class='btn btn-link '><i class="icon-common-edit icon-edit"></i> 编辑</a>
						<a href="javascript:void(0)" onclick="del(${p.id})" class='btn btn-link '><i class="icon-common-delete icon-trash"></i> 删除</a>
						<a href="team/need/toaddproduct?product_id=${p.id}" class='btn btn-link '><i class="icon icon-plus"></i> 提需求</a>
					</div>
				</div>
			</div>
		</main>
