							...
							<tbody>
								...
								<tr>
									<td colspan="3" class="text-center form-actions">
										<button id="submit_yes" class="btn btn-wide btn-primary" data-loading="稍候...">通过</button>
										<button id="submit_no" class="btn btn-wide btn-primary" data-loading="稍候...">不通过</button>
...
$("#submit_yes").click(function(){
	...
	$.ajax({type:"POST",url:"team/task/finishCheck?is=1&r=" + Math.random(),data:$("form").serialize() + "&checked_reason=" + UM.getEditor('checked_reason').getContent(),dataType:"json",success:function(data){
		if(data.code == 0){
...
$("#submit_no").click(function(){
	$.ajaxSettings.async = false;
	$.ajax({type:"POST",url:"team/task/finishCheck?is=0&r=" + Math.random(),data:$("form").serialize(),dataType:"json",success:function(data){
		if(data.code == 0){
			$("#msg").text(data.message);
			$('#myModal').modal({backdrop: 'static', keyboard: false,show: true, moveable: true});
		}else{
